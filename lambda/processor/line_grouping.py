def group_blocks(blocks: list) -> tuple[list[list], float, float]:
    ...
    Phase 3 — Absorb price orphans: any single-block row whose block sits
    in the right-hand price column (X > 65 %) and couldn't connect during
    chaining is merged into the nearest row within 2 × step_tol.  Rows below
    the orphan (higher Y) are preferred over rows above, because receipt curl
    pushes the right edge physically upward so its Textract Y is lower than
    the other blocks on the same logical row.
    ...
    # Phase 3 — absorb isolated right-edge price blocks
    i = 0
    while i < len(rows):
        if len(rows[i]) != 1 or _left(rows[i][0]) <= 0.65:
            i += 1
            continue
        orphan_y = _top(rows[i][0])
        best_j, best_eff = None, float("inf")
        for j, other in enumerate(rows):
            if j == i:
                continue
            other_y = sum(_top(b) for b in other) / len(other)
            gap = abs(other_y - orphan_y)
            if gap > step_tol * 2:
                continue
            # Rows below the orphan (other_y > orphan_y) are preferred:
            # receipt curl makes the price column appear higher than its row.
            eff = gap if other_y > orphan_y else gap + step_tol
            if eff < best_eff:
